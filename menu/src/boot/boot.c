...
// The single definition of the global declared `extern` in boot.h.
...
void boot (boot_params_t *params) {
    ...
    /* Copy the first 1 MB of game code (ROM offset 0x1000, after header + IPL3)
       to RDRAM at 0x80001000, replicating what IPL3 does on a real cartridge. */
    void *rdram_dst = (void *)0x80001000;
    uint32_t cart_src = (uint32_t)(&base[0x400]); // io32_t* so [0x400] = byte offset 0x1000

    bool cheats_installed = cheats_install(cic_type, params->cheat_list);
    
    debugf("cheats installed: %i\n", cheats_installed);
