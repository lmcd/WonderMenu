...
/* Unsigned divide rs/rt -> LO (quotient), HI (remainder). Read LO with I_MFLO. */
#define I_DIVU(rs, rt) __ASM_R_INST(OP_SPECIAL, rs, rt, 0, 0, FUNCT_DIVU)
#define I_SW(rt, offset, base) __ASM_I_INST(OP_SW, base, rt, offset)
#define I_MFHI(rd) __ASM_R_INST(OP_SPECIAL, REG_ZERO, REG_ZERO, rd, 0, FUNCT_MFHI)
#define I_MFLO(rd) __ASM_R_INST(OP_SPECIAL, REG_ZERO, REG_ZERO, rd, 0, FUNCT_MFLO)
#define I_MTHI(rs) __ASM_R_INST(OP_SPECIAL, rs, REG_ZERO, REG_ZERO, 0, FUNCT_MTHI)
#define I_MTLO(rs) __ASM_R_INST(OP_SPECIAL, rs, REG_ZERO, REG_ZERO, 0, FUNCT_MTLO)
/* Store FPU register ft (0-31) to offset(base). Needs Status.CU1 enabled. */
#define I_SWC1(ft, offset, base) __ASM_I_INST(OP_SWC1, base, ft, offset)
/* Move FPU control reg fs (e.g. 31 = FCR31) to GPR rt. rs field 2 = CFC1. */
#define I_CFC1(rt, fs) __ASM_R_INST(OP_COP1, 2, rt, fs, 0, 0)
